...
namespace Hangfire
{
   internal class RecurringJobEntity
   {
      ...
      private IReadOnlyDictionary<string, string> GetChangedFields(out DateTime? nextExecution)
      {
         ...
         // customized - Redsand
         var serializedFakeHttpRequestContext = FakeHttpRequestContext is string v ? v : SerializationHelper.Serialize(FakeHttpRequestContext, SerializationOption.User);
